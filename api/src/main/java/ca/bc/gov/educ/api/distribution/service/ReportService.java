...
public class ReportService {
    ...
    public InputStreamResource getPackingSlip(ReportRequest packingSlipReq) {
        log.debug("Getting packing slip for order {}", packingSlipReq.getData().getPackingSlip().getOrderNumber());
        if (log.isDebugEnabled()) {
            String packingSlipJson = jsonTransformer.marshall(packingSlipReq);
            log.debug(packingSlipJson);
        }
    ...
    public ReportRequest preparePackingSlipData(StudentSearchRequest searchRequest, ca.bc.gov.educ.api.distribution.model.dto.v2.School schoolDetails, Long batchId) {
        ...
        if (useSchoolAddress) {
            ca.bc.gov.educ.api.distribution.model.dto.v2.School school = schoolService.getSchool(UUID.fromString(schoolDetails.getSchoolId()), new ExceptionMessage());
            if (school != null) {
